  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
            data.status === "healthy" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
          }`}>
            {data.status}
          </span>
        </div>
      ...
      <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
        <h2 className="text-sm font-bold text-slate-700 mb-3">Today&apos;s Activity</h2>
      ...
      {Object.keys(data.sourceStats).length > 0 && (
        <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
          <h2 className="text-sm font-bold text-slate-700 mb-3">Source Performance (Today)</h2>
      ...
      {data.locks.length > 0 && (
        <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
          <h2 className="text-sm font-bold text-slate-700 mb-3">Active Locks</h2>
          <div className="space-y-2">
            {data.locks.map((lock) => (
              <div key={lock.name} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
                <span className="text-xs font-medium text-slate-600">{lock.name}</span>
                <span className={`text-xs font-semibold ${lock.isRunning ? "text-amber-600" : "text-emerald-600"}`}>
      ...
      {data.recentErrors.length > 0 && (
        <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-red-100">
          <div className="flex items-center gap-2 mb-3">
            <AlertTriangle className="h-4 w-4 text-red-500" />
            <h2 className="text-sm font-bold text-red-700">Recent Errors</h2>
          </div>
          <div className="space-y-2">
            {data.recentErrors.map((err, i) => (
              <div key={i} className="rounded-lg bg-red-50 px-3 py-2 text-xs">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-red-700">{err.source || "system"}</span>
                  <span className="text-red-400">{timeAgo(err.time)}</span>
                </div>
                <p className="text-red-600 mt-0.5 truncate">{err.message}</p>
              </div>
            ))}
          </div>
        </div>
