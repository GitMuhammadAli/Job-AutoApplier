    ...
    <div className="space-y-8 max-w-3xl">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-sm font-bold text-slate-800">{provider.label}</span>
                  {provider.badge && (
                ...
                <p className="text-[11px] text-slate-500 mb-2.5">{provider.description}</p>
                ...
                <div className={`rounded-lg p-2.5 ring-1 text-[10px] ${provider.previewBg}`}>
                  ...
                  <div className="mt-1 text-slate-500">{provider.hrResult}</div>
                </div>
            ...
            {emailProvider === "gmail" && (
              <div className="rounded-lg bg-blue-50 p-3 ring-1 ring-blue-100">
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-semibold text-blue-700 touch-manipulation w-full"
                >
                  {helpExpanded ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
                  How to get a Gmail App Password
                </button>
                {helpExpanded && (
                  <ol className="mt-2 space-y-1 text-[11px] text-blue-600 list-decimal list-inside">
      ...
      <Section icon={<Send className="h-4 w-4" />} title="Application Mode">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <div className={`rounded-lg p-1.5 ${isActive ? "bg-violet-100 text-violet-600" : "bg-slate-100 text-slate-500"}`}>
                    {MODE_ICONS[mode.icon]}
                  </div>
                  <div>
                    <span className="text-sm font-bold text-slate-800">{mode.label}</span>
                    {mode.badge && (
                ...
                <p className="text-[11px] font-medium text-slate-600 mb-1.5">{mode.shortDesc}</p>
                <p className="text-[11px] text-slate-500 mb-2">{mode.description}</p>
        ...
        {selectedMode && (
          <div className="mt-4 rounded-xl bg-slate-50 p-4 ring-1 ring-slate-100">
            <h4 className="text-xs font-bold text-slate-700 mb-2">What happens in {selectedMode.label} mode:</h4>
            <ol className="space-y-1">
              {selectedMode.steps.map((step, i) => (
                <li key={i} className="flex items-start gap-2 text-[11px] text-slate-600">
                  <span className="flex h-4 w-4 items-center justify-center rounded-full bg-violet-100 text-[9px] font-bold text-violet-600 flex-shrink-0 mt-0.5">
                    {i + 1}
                  </span>
                  {step}
                </li>
              ...
              {readinessChecks
                ...
                .map((check) => (
                  <div key={check.key} className="flex items-center gap-2 text-[11px]">
                    ...
                    <span className={check.met ? "text-slate-600" : "text-red-600 font-medium"}>
                      {check.label}
                    </span>
                  </div>
                ))}
        ...
        {(applicationMode === "full_auto" || applicationMode === "instant") && (
          <div className="mt-4 space-y-4 rounded-xl border border-violet-200 bg-violet-50/30 p-4">
            <div className="flex items-center justify-between">
              <div>
      ...
      <Section icon={<Shield className="h-4 w-4" />} title="Sending Safety">
        ...
        {s.sendingPausedUntil && new Date(s.sendingPausedUntil) > new Date() && (
          <div className="mb-4 rounded-lg bg-red-50 p-3 ring-1 ring-red-200 text-xs text-red-700 font-medium flex items-center gap-2">
          ...
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="py-2 pr-3 text-left font-semibold text-slate-500">Feature</th>
                <th className="py-2 px-3 text-center font-semibold text-slate-500">Manual</th>
                <th className="py-2 px-3 text-center font-semibold text-slate-500">Semi-Auto</th>
                <th className="py-2 px-3 text-center font-semibold text-slate-500">Full Auto</th>
                <th className="py-2 pl-3 text-center font-semibold text-slate-500">Instant</th>
              </tr>
            </thead>
