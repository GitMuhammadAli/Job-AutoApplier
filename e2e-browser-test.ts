...
async function main() {
  ...
  report.push("- **Auth state:** " + (useAuth ? "Loaded from .auth/user.json" : "None (fresh browser)"));
  report.push("");
  ...
  try {
    ...
    if (isLogin) {
      ...
      report.push("- **Login page elements:** Google=" + hasGoogle + ", GitHub=" + hasGitHub + ", Email=" + hasEmail);
    } else {
      ...
      report.push("- **Expected jobs (Full Stack, Senior React):** " + (hasFullStack || hasSeniorReact ? "Yes" : "No"));
      report.push("- **Excluded jobs (Copywriter, Inside Sales, Office Assistant):** " + (hasCopywriter || hasInsideSales || hasOfficeAssistant ? "FOUND (should NOT appear)" : "Correctly absent"));
      const hasMatchScore = bodyText.includes("%") || bodyText.includes("Match");
      const hasSourceBadge = bodyText.includes("LinkedIn") || bodyText.includes("Indeed") || bodyText.includes("JSearch");
      report.push("- **Match score visible:** " + (hasMatchScore ? "Yes" : "Unknown"));
      report.push("- **Source badge visible:** " + (hasSourceBadge ? "Yes" : "Unknown"));
    }
    report.push("");
    ...
    report.push("- **Result:** " + (appsIsLogin ? "Redirected to login" : "Page loaded"));
    report.push("- **URL:** " + url2);
    if (!appsIsLogin) {
      const hasTabs = await page.locator('[role="tablist"], [data-state="active"]').count() > 0;
    ...
    report.push("- **Result:** " + (templatesIsLogin ? "Redirected to login" : "Page loaded"));
    report.push("- **URL:** " + url3);
    if (!templatesIsLogin) {
      const templateCount = await page.locator("text=Professional Standard, text=Confident, text=Referral, text=Short").count();
      ...
      report.push("- **Templates visible:** Professional=" + hasProfessional + ", Confident=" + hasConfident);
    }
    report.push("");
    ...
    report.push("- **Result:** " + (analyticsIsLogin ? "Redirected to login" : "Page loaded"));
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, "05-settings.png"), fullPage: true });
    const url5 = page.url();
    const settingsIsLogin = url5.includes("/login");
    report.push("- **Result:** " + (settingsIsLogin ? "Redirected to login" : "Page loaded"));
    report.push("- **URL:** " + url5);
    if (!settingsIsLogin) {
      const hasForm = await page.locator("form, input, button").count() > 0;
      report.push("- **Settings form visible:** " + (hasForm ? "Yes" : "Unknown"));
    }
    report.push("");
    ...
    report.push("- **Theme:** " + (htmlClass.includes("dark") ? "Dark" : "Light"));
  } catch (err) {
    report.push("## Error");
    report.push("```");
