...
async function main() {
  ...
  try {
    ...
    report.push(
      "- **Loaded:** " + (isLogin1 ? "No - redirected to login" : "Yes"),
    );
    if (!isLogin1) {
      ...
      report.push(
        "- **Cards show title, company:** Yes (implied by job cards)",
      );
      report.push(
        "- **Match score visible:** " + (hasMatchScore ? "Yes" : "No"),
      );
      report.push(
        "- **Source badge visible:** " + (hasSourceBadge ? "Yes" : "No"),
      );
      report.push(
        "- **Excluded (Copywriter, Inside Sales, Office Assistant):** " +
    ...
    report.push(
      "- **Loaded:** " + (isLogin2 ? "No - redirected to login" : "Yes"),
    );
    if (!isLogin2) {
      const bodyText = (await page.textContent("body")) || "";
      ...
      const hasSendingStatusBar =
        bodyText.includes("Sending") || bodyText.includes("sending");
      const hasEmptyState =
        bodyText.includes("No applications") || bodyText.includes("empty");
      const hasCards =
        (await page
          .locator('[class*="rounded"]')
          .filter({ hasText: /Draft|Ready|Sent/ })
          .count()) > 0;
      report.push(
        "- **Tabs (All, Drafts, Ready, Sent, Failed, Bounced):** All=" +
          hasAll +
          ", Draft=" +
          hasDrafts +
          ", Ready=" +
      ...
      report.push(
        "- **SendingStatusBar visible:** " +
          (hasSendingStatusBar ? "Yes" : "Unknown"),
      );
      report.push(
        "- **Content:** " +
          (hasCards
    ...
    report.push(
      "- **Loaded:** " + (isLogin3 ? "No - redirected to login" : "Yes"),
    );
    if (!isLogin3) {
      const bodyText = (await page.textContent("body")) || "";
      const templateCount = (
        bodyText.match(/Professional|Confident|Referral|Short/g) || []
      ).length;
      const hasDefault =
      ...
      const hasCreateNew =
        ...
        bodyText.includes("Create new") ||
        bodyText.includes("Add Template");
      report.push(
        "- **Templates visible:** " + templateCount + " (expected 3+)",
      );
      report.push("- **DEFAULT badge/star:** " + (hasDefault ? "Yes" : "No"));
      report.push(
        "- **Create New Template button:** " + (hasCreateNew ? "Yes" : "No"),
      );
    }
    report.push("");
    ...
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, "04-resumes.png"),
      fullPage: true,
    });
    const url4 = page.url();
    const isLogin4 = url4.includes("/login");
    report.push(
      "- **Loaded:** " + (isLogin4 ? "No - redirected to login" : "Yes"),
    );
    if (!isLogin4) {
      const bodyText = (await page.textContent("body")) || "";
      const hasUpload =
        ...
        bodyText.includes("Add Resume");
      const hasResumes =
        bodyText.includes("resume") &&
        (bodyText.includes("variant") || bodyText.includes("Resume"));
      report.push(
        "- **Resume upload area:** " + (hasUpload ? "Yes" : "Unknown"),
      );
      report.push(
        "- **Content:** " +
          (hasResumes ? "Resumes listed or upload UI" : "Empty or other"),
      );
    }
    report.push("");
    ...
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, "05-analytics.png"),
      fullPage: true,
    });
    const url5 = page.url();
    const isLogin5 = url5.includes("/login");
    report.push(
      "- **Loaded:** " + (isLogin5 ? "No - redirected to login" : "Yes"),
    );
    if (!isLogin5) {
      ...
      const hasStatCards =
        ...
        bodyText.includes("Matches") ||
        bodyText.includes("Sent");
      report.push(
        "- **Charts (SVG) render:** " +
          (svgCount > 0 ? "Yes (" + svgCount + " SVGs)" : "No/blank"),
      );
      report.push(
        "- **Stat cards visible:** " + (hasStatCards ? "Yes" : "Unknown"),
      );
    }
    report.push("");
    ...
    report.push(
      "- **Loaded:** " + (isLogin6 ? "No - redirected to login" : "Yes"),
    );
    if (!isLogin6) {
      const bodyText = (await page.textContent("body")) || "";
      const hasAliShahid =
        bodyText.includes("Ali Shahid") || bodyText.includes("Ali");
      const hasKeywords =
        bodyText.includes("keyword") || bodyText.includes("Keywords");
      const hasSections =
        bodyText.includes("Profile") ||
        bodyText.includes("Job") ||
        bodyText.includes("Email");
      report.push(
        "- **Form with data (Ali Shahid, keywords):** " +
          (hasAliShahid ? "Yes" : "No"),
      );
      report.push(
        "- **All sections visible:** " + (hasSections ? "Yes" : "Unknown"),
      );
    }
    report.push("");
    ...
    const themeToggle = page
      .locator(
        'button[aria-label*="theme"], button[aria-label*="Theme"], [data-state][class*="theme"], button:has(svg)',
      )
      .first();
    const sunMoon = page
      .locator(
        'button:has([data-lucide="Sun"]), button:has([data-lucide="Moon"]), [class*="theme"] button',
      )
      .first();
    ...
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, "07-dark-mode.png"),
      fullPage: true,
    });
    ...
    report.push(
      "- **Theme toggle clicked:** " + (darkModeClicked ? "Yes" : "No"),
    );
    report.push("- **HTML class after toggle:** " + (htmlClass || "(none)"));
    report.push(
      "- **Dark mode active:** " + (htmlClass.includes("dark") ? "Yes" : "No"),
    );
    report.push("");

    // PAGE 8: Error Page
    report.push("## Page 8: Error Page (404)");
    await page.goto(`${BASE_URL}/jobs/nonexistent-invalid-id`, {
      waitUntil: "networkidle",
      timeout: 15000,
    });
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, "08-error-page.png"),
      fullPage: true,
    });
    const bodyText8 = (await page.textContent("body")) || "";
    const has404 =
      bodyText8.includes("404") ||
      bodyText8.includes("Not Found") ||
      bodyText8.includes("not found");
    ...
    report.push(
      "- **Shows 404/error:** " + (has404 || hasError ? "Yes" : "No"),
    );
    ...
    if (linkCount > 0) {
      ...
      if (url9.includes("/jobs/") && !url9.includes("nonexistent")) {
        await page.screenshot({
          path: path.join(SCREENSHOT_DIR, "09-job-detail.png"),
          fullPage: true,
        });
        ...
        const hasCompany =
          bodyText9.includes("company") || bodyText9.includes("Company");
        const hasLocation =
          bodyText9.includes("location") || bodyText9.includes("Location");
        const hasSalary =
          bodyText9.includes("salary") ||
          bodyText9.includes("Salary") ||
          bodyText9.includes("$");
        const hasDescription =
          bodyText9.includes("description") ||
          bodyText9.includes("Description") ||
          bodyText9.includes("requirements");
        const hasQuickApply =
          bodyText9.includes("Quick") ||
          bodyText9.includes("Apply") ||
          bodyText9.includes("email") ||
          bodyText9.includes("Generate");
        report.push("- **Job detail loaded:** Yes");
        report.push(
          "- **Left panel (title, company, location, salary, description):** " +
            (hasTitle && (hasCompany || hasLocation) ? "Yes" : "Partial"),
        );
        report.push(
          "- **Right panel QuickApplyPanel / email generation:** " +
            (hasQuickApply ? "Yes" : "Unknown"),
        );
      }
    }
    if (!jobDetailLoaded) {
